...
export default function ScalePage({ name, author, id,ownerId }: props) {
  const {levels,setLevels,addMovie} = useLevels();

  return (
    <Layout>
      <div className={classes.container} >
        <div className={classes.title}>
          <h1 className="title"><div className={classes.scaleName}>{name}</div></h1>
          <h2>{author}</h2>
          <MovieCount levels={levels}/>
        </div>
        <div className={classes.scaleContainer}>
          <div className={classes.scale} id='scale'>
            <Scale scaleId={id} ownerId={ownerId} levels={levels} setLevels={setLevels} addMovie={addMovie}/>
          </div>
        </div>
        <Trash />
      </div>
